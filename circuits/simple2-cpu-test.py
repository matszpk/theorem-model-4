...
ap.add_argument('--no-subcircuits', type=lambda x: int(x,0), default=0)
args = ap.parse_args()
...
def cpu_main_phase23_2_t2_input_test_func(case):
    instr = 14+(case&1) if (case&2)!=0 else 8+(case&1)
    return bin_comp(cpu_main_input_str,
        {'phase':2,'instr':instr,'tmp':11, 'acc':(case>>2)&0xff, 'flags':((case>>10)&0x1)|6,
            'pc':0xb1a,'mem_value':(case>>11)&0xff})
def cpu_main_phase23_3_t2_input_test_func(case):
    instr = 14+(case&1) if (case&2)!=0 else 8+(case&1)
    return bin_comp(cpu_main_input_str,
        {'phase':3,'instr':instr,'tmp':11, 'acc':(case>>2)&0xff, 'flags':((case>>10)&0x1)|6,
            'pc':0xb1a,'mem_value':(case>>11)&0xff})
...
gen_testsuite("cpu_main_phase23_2_t2", "main", 42, 57, range(0, 1<<19), cpu_main,
                cpu_main_phase23_2_t2_input_test_func)
gen_testsuite("cpu_main_phase23_3_t2", "main", 42, 57, range(0, 1<<19), cpu_main,
                cpu_main_phase23_3_t2_input_test_func)
